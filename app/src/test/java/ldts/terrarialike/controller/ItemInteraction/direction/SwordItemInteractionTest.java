...
public class SwordItemInteractionTest {
    ...
    Position position2 = Mockito.mock(Position.class);
        ...
        Mockito.when(position1.getX()).thenReturn(3);
        Mockito.when(position1.getY()).thenReturn(3);
        Mockito.when(position2.getX()).thenReturn(4);
        Mockito.when(position2.getY()).thenReturn(4);
    ...
    SwordItemInteraction swordItemInteraction = new SwordItemInteraction(5,5);

    List<GameEvent> gameEvents = swordItemInteraction.execute(world, InteractionType.ATTACK, item, worldUtils);

        Assertions.assertEquals(new ArrayList<>(), gameEvents);


        swordItemInteraction.setDirection(true);
